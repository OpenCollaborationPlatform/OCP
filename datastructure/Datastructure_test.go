...
func TestDatastructure(t *testing.T) {
	...
	//share the dml file to be accessbile for the datastrcuture
	...
	dmlCid, _ := host.Data.Add(context.Background(), dmlpath)

	Convey("Setting up a new datastructure works", t, func() {
		
		ctx, _ := context.WithTimeout(context.Background(), 5*time.Second)
		ds, err := NewDatastructure(ctx, dmlCid, host, dsClient, swarmpath)
		So(err, ShouldBeNil)
		...
		Convey("And events are forwarded to all clients", func() {
			...
			testClient.Subscribe("Test/TestEventZeroArgs", handlerZeroArgs, make(wamp.Dict, 0))
			...
			testClient.Subscribe("Test/TestEventTwoArgs", handlerTwoArgs, make(wamp.Dict, 0))
